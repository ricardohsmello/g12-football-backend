...
public class RagIngestDataUseCase extends AbstractUseCase<RagIngestDataRequest> {
    ...
    public RagIngestDataResponse execute(RagIngestDataRequest ragIngestDataRequest) {
        predictedHome = ragIngestDataRequest.predictedHome();
        predictedAway = ragIngestDataRequest.predictedAway();

        actualHome = ragIngestDataRequest.actualHome();
        actualAway = ragIngestDataRequest.actualAway();

        homeTeam = ragIngestDataRequest.homeTeam();
        awayTeam = ragIngestDataRequest.awayTeam();
        username = ragIngestDataRequest.username();
        round = ragIngestDataRequest.round();

        pointsEarned = ragIngestDataRequest.pointsEarned();
        roundPoints = ragIngestDataRequest.roundPoints();
        totalPoints = ragIngestDataRequest.totalPoints();
        ...
        return new RagIngestDataResponse(
                String.join(". ", phrases) + ".",
                getStringObjectMap(ragIngestDataRequest)
        );
    }

    private Map<String, Object> getStringObjectMap(RagIngestDataRequest ragIngestDataRequest) {
        Map<String, Object> md = new LinkedHashMap<>();
        md.put("type", "bet_join_embedding");
        md.put("username", username);
        md.put("round", round);
        md.put("betDate", ragIngestDataRequest.betDate());
